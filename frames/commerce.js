...
async function profileView() {
  ...
  show(`<div class="panel-head"><p class="eyebrow">Shipping Profile</p><h2>Company information</h2></div><form class="commerce-form two-col" id="profileForm"><input type="hidden" name="buyer_type" value="company"><label>Manager / Contact name<input name="full_name" required value="${e(profile.full_name)}"></label><label>Company name<input name="company_name" required value="${e(profile.company_name)}"></label><label>Phone<input name="phone" required value="${e(profile.phone)}"></label><label>WhatsApp<input name="whatsapp" value="${e(profile.whatsapp)}"></label><label>Country<input name="country" required value="${e(profile.country)}"></label><label>Postal code<input name="postal_code" required value="${e(profile.postal_code)}"></label><label class="wide">Address line 1<input name="address_line_1" required value="${e(profile.address_line_1)}"></label><label class="wide">Address line 2<input name="address_line_2" value="${e(profile.address_line_2)}"></label><label>City<input name="city" required value="${e(profile.city)}"></label><label>State / Province<input name="state_province" value="${e(profile.state_province)}"></label><label>Importer / Customs ID (optional)<input name="tax_id" value="${e(profile.tax_id)}"><small>Enter the importer or customs identification number requested by your local customs authority or courier (for example, an EORI or Importer Number). This is not a general business registration number. Leave it blank if it is not required for your shipment.</small></label><label>Preferred courier<select name="preferred_courier"><option value="">No collect account</option>${['DHL', 'FedEx', 'UPS', 'EMS', 'SF Express', 'Other'].map(value => `<option ${profile.preferred_courier === value ? 'selected' : ''}>${value}</option>`).join('')}</select></label><label class="wide">Courier collect account (optional)<input name="courier_account_no" value="${e(profile.courier_account_no)}"></label><label class="wide reminder-consent"><input name="cart_reminder_opt_in" type="checkbox" value="true" ${[true, 'true', 1, '1'].includes(profile.cart_reminder_opt_in) ? 'checked' : ''}><span>Email me reminders about items left in my cart. I can turn these reminders off at any time.</span></label><button class="button wide">Save profile</button><p class="form-status wide" id="profileStatus"></p></form>`);
...
function cartView() {
  ...
  show(`<div class="panel-head cart-heading"><div><p class="eyebrow">Shopping Cart</p><h2>${cart.length ? 'Your cart' : 'Your cart is empty'}</h2></div><span>${cart.reduce((sum, item) => sum + item.quantity, 0)} items</span></div>
    ...
    ${cart.length ? `<div class="cart-summary"><div><span>${hasQuote ? 'Priced items subtotal' : 'FOB China subtotal'}</span><strong>USD ${money(total)}</strong></div><p>Freight, destination duties and local taxes are not included. Availability and freight are confirmed before the Proforma Invoice is issued.</p>${belowMinimum ? `<p class="minimum-order-note">Minimum product order is USD 100. Please add USD ${money(100-total)} more.</p>` : '<p class="minimum-order-note ready">Minimum order reached.</p>'}</div><div class="cart-actions"><button class="button secondary-button" id="continueShopping">Continue shopping</button><button class="button" id="checkoutButton" ${belowMinimum ? 'disabled' : ''}>Proceed to checkout</button></div><p class="form-status" id="quoteStatus"></p>` : `<button class="button" id="continueShopping">Continue shopping</button>`}`, true);
...
async function submitQuote(event) {
  event.preventDefault();
  const currentSubtotal = cart.reduce((sum, item) => sum + (item.priceUsd || 0) * item.quantity, 0);
  if (currentSubtotal < 100) { cartView(); return; }
  ...
  if (!profile?.company_name || !profile?.full_name || !profile?.country || !profile?.address_line_1 || !profile?.postal_code) {
    status.textContent = 'Please complete your company and shipping profile first.';
...
window.addEventListener('lzn:add-cart', event => {
  ...
  const found = cart.find(item => item.model === model && item.pd === pd && item.pdLabel === pdLabel && item.optionLabel === optionLabel);
  if (found) found.quantity += 1;
  else cart.push({ model, nameEn: product.nameEn, image: option?.image || product.image, priceUsd, pd, pdLabel, optionLabel, quantity: 1 });
  save();
  document.querySelector('#modal [data-close]')?.click();
  toast(`${model} added to cart.`, true);
});
...
if (client) {
  ...
  client.auth.onAuthStateChange((event, newSession) => {
    ...
    session = newSession;
    accountLabel();
    if (event === 'SIGNED_IN' && !wasSignedIn) toast(`Signed in as ${session.user.email}`);
