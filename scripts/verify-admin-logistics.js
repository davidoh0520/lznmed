...
(async () => {
  ...
  if (total !== 2023) throw new Error(`Expected 2023 root and option-level catalog rows, found ${total}.`);
  ...
  if (await page.locator('#logisticsModel option').count() !== 242) throw new Error('Device model selector does not contain all 241 root, option-level and legacy device models.');
  for (const model of ['AST-1000', 'HRT-3', 'C6', 'C11', 'RT9000T']) {
    if (!(await page.locator(`#logisticsModel option[value="${model}"]`).count())) throw new Error(`Legacy device model ${model} is missing from the selector.`);
  }
