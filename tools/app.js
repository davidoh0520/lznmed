...
function openProduct(model) {
  ...
  modalBody.innerHTML = `<div class="detail-media"><div class="detail-image"><img id="detailMainImage" src="${esc(images[0])}" alt="${esc(product.model)}" decoding="async"></div>${images.length > 1 ? `<div class="detail-thumbs">${images.map((src, index) => `<button class="${index === 0 ? 'active' : ''}" data-gallery-src="${esc(src)}"><img src="${esc(src)}" alt="${esc(product.model)} view ${index + 1}" loading="lazy" decoding="async"></button>`).join('')}</div>` : ''}</div>
