...
function openAccount() {
  if (accountSession) {
    accountBody.innerHTML = `<p class="kicker"><span></span> Company account</p><h2>My Account</h2><p>Signed in as <strong>${escapeHtml(accountSession.user.email)}</strong></p><div class="account-actions"><a href="/tools/" class="button primary">Open account & orders</a><button class="button account-signout" id="accountSignOut">Sign out</button></div>`;
...
async function handleAccount(event) {
  ...
  const result = mode === 'signup'
    ? await accountClient.auth.signUp({email:form.get('email'),password:form.get('password'),options:{emailRedirectTo:location.origin + '/?email-confirmed=1',data:{company_name:String(form.get('company_name')).trim(),full_name:String(form.get('full_name')).trim(),buyer_type:'company',cart_reminder_opt_in:form.get('cart_reminder_opt_in')==='true'}}})
