...
function render(filter='all'){
  ...
  products.filter(p=>filter==='all'||p.cat===filter).forEach(p=>{
    ...
    const priceMarkup=p.cat==='semi'
      ...
      : `<div class="card-price">From <strong>USD ${fromPrice(p).toFixed(2)}</strong> <small>/ lens</small></div>`;
...
function updateOrderPrice(){
  ...
  orderPrice.innerHTML=price===undefined?'Combination unavailable':`USD ${price.toFixed(2)} × ${qty} lens${qty===1?'':'es'} = <strong>USD ${(price*qty).toFixed(2)}</strong>`;
...
function cartView(){
  ...
  showCommerce(`<p class="eyebrow">BULK LENS ORDER</p><h2>${cart.length?'Your Cart':'Your cart is empty'}</h2><p class="order-note">Unit prices are per lens. Standard export packing uses plain white envelopes marked with the lens type and prescription power. Availability and shipping are confirmed before invoicing.</p><div class="cart-list">${cart.length?cart.map((x,i)=>`<div class="cart-line"><div><strong>${e(x.name)}</strong><span>${e(linePower(x))}</span><small>USD ${money(x.price)} each</small></div><label class="cart-qty">Qty<input data-qty="${i}" type="number" min="1" step="1" value="${x.qty}"></label><div><b>USD ${money(x.price*x.qty)}</b><button data-remove="${i}" aria-label="Remove">&times;</button></div></div>`).join(''):'<p class="empty-cart">Choose a product and add a lens power.</p>'}</div>${cart.length?`<div class="cart-summary"><span>${totalQty} lenses</span><strong>USD ${money(total)}</strong></div><div class="commerce-actions"><button class="btn secondary" id="continueShopping">Continue Shopping</button><button class="btn" id="checkoutButton">Proceed to Checkout</button></div>`:'<button class="btn" id="continueShopping">Continue Shopping</button>'}`);
